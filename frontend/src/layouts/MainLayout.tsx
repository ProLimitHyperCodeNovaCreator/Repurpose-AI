...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-b text-slate-800 dark:text-slate-100 from-slate-100/90 via-slate-50 to-slate-100/80 dark:from-black dark:via-black dark:to-black dark:bg-black">
      <div className="hidden lg:block"><Sidebar /></div>

      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileMenuOpen(false)} className="lg:hidden fixed inset-0 bg-slate-900/25 dark:bg-black/50 z-40" />
            <motion.div initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: 'spring', bounce: 0, duration: 0.3 }} className="lg:hidden fixed left-0 top-0 bottom-0 z-50">
              <div className="relative">
                <Sidebar />
                <button onClick={() => setMobileMenuOpen(false)} className="absolute top-4 right-4 p-2 text-slate-500 hover:text-slate-800 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </motion.div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main ref={mainRef} className="flex-1 overflow-auto pb-20 lg:pb-0 min-h-0 dark:bg-black">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="p-4 lg:p-6 min-h-full dark:bg-black">
            <Outlet />
          </motion.div>
        </main>
        <MobileNav />
